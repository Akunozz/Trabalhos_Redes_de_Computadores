...
# Itera sobre os pacotes e conta o tráfego por intervalo de tempo
for pacote in pacotes:
    tempo = pacote.time
    intervalo_tempo = int(tempo) // intervalo * intervalo
    if intervalo_tempo in trafego_por_intervalo:
        trafego_por_intervalo[intervalo_tempo] += 1
...
line = (
    ...
    .add_yaxis("Número de Pacotes", [y[1] for y in dados_grafico], symbol='circle', is_symbol_show=True)
    .set_series_opts(label_opts=opts.LabelOpts(is_show=False))
    .set_global_opts(
        xaxis_opts=opts.AxisOpts(name="Tempo", axislabel_opts={"rotate": 45}),
        yaxis_opts=opts.AxisOpts(name="Número de Pacotes"),
        title_opts=opts.TitleOpts(title=f"Tráfego por intervalo de {intervalo} segundo(s)"),
    )
)

# Renderiza o gráfico
line.render("grafico_echarts.html")
